...
fn token_out(ctx: &impl ReadContext, token_in: &ContractAddress) -> Result<ContractAddress, Error> {
    let token_a = ctx.model().token_a(ctx);
    let token_b = ctx.model().token_b(ctx);
    if token_in == &token_a {
        Ok(token_b)
...
fn quote_swap(
    ...
    let custodian = ctx.model().custodian(ctx);
    ...
    calc_swap_result(amount_in, bal_in, bal_out, ctx.model().fee_bps(ctx))
}
...
fn quote_deposit(
    ...
    let token_a = ctx.model().token_a(ctx);
    let token_b = ctx.model().token_b(ctx);
    let lp_supply = ctx.model().lp_total_supply(ctx);
...
impl Guest for Pool {
    ...
    fn re_init(
        ...
        Ok(ctx.model().lp_total_supply(ctx))
    }

    fn fee(ctx: &ViewContext) -> Integer {
        ctx.model().fee_bps(ctx)
    }

    fn balance(ctx: &ViewContext, acc: String) -> Option<Integer> {
        ctx.model().lp_ledger().get(ctx, acc)
    }
    ...
    fn token_balance(ctx: &ViewContext, token: ContractAddress) -> Result<Integer, Error> {
        token_out(ctx, &token)?;
        Ok(token_dyn::balance(&token, &ctx.model().custodian(ctx)).unwrap_or_default())
    }
    ...
    fn deposit(
        ...
        ctx.model().set_lp_total_supply(ctx, ctx.model().lp_total_supply(ctx) + res.lp_shares);

        token_dyn::transfer(
            &ctx.model().token_a(ctx),
            ctx.signer(),
            &custodian,
            res.deposit_a,
        )?;
        token_dyn::transfer(
            &ctx.model().token_b(ctx),
            ctx.signer(),
            &custodian,
            res.deposit_b,
        )?;
    ...
    fn withdraw(ctx: &ProcContext, shares: Integer) -> Result<WithdrawResult, Error> {
        ...
        let total = ctx.model().lp_total_supply(ctx);
        let bal = ledger.get(ctx, &user).unwrap_or_default();
        ...
        ctx.model().set_lp_total_supply(ctx, total - shares);
