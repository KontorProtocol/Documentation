...
mod tests {
    ...
    async fn test_contract() -> Result<()> {
        ...
        let res = token_dyn::balance(runtime, &pool, &admin).await?;
        assert_eq!(res, Some(223.into()));
        let res = token_dyn::balance(runtime, &pool, &minter).await?;
        assert_eq!(res, None);

        token_dyn::transfer(runtime, &pool, &admin, &minter, 23.into()).await??;
