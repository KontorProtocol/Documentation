...
sequenceDiagram
    ...
    BC->>IX: Mine Block (height h)
    IX->>IX: Generate Challenges
    SN-->>IX: Fetch Challenges
    
    SN->>SN: Generate Nova IVC Proof
    SN->>BC: Submit Proof Tx
    
    BC->>IX: Mine Block (height h+k)
    IX->>IX: Verify Proof
    IX->>IX: Process Expired (h+W_proof)
```
