...
Install Rust and the Sigil SDK, then follow the [getting started guide](/docs/sigil/getting-started) in the documentation. The SDK includes example contracts (token, NFT, simple DeFi primitives) that demonstrate common patterns. The SDK provides testing utilities so you can write unit and integration tests that execute locally before deploying. Join the developer community for support and to see what others are building!
...
Alkanes and Kontor are both Bitcoin metaprotocols with WebAssembly smart contracts, but they have very different functionality in practice. Kontor's a hybrid accounts/UTXO model lets Kontor avoid significant concurrency that are a consequence of Alkanes' reliance on UTXOs for contract execution. Kontor's adoption of the WebAssembly Component Model makes invocation of contracts and cross-contract calls dramatically simpler, with strong type safety everywhere. Kontor also has KOR as a native currency with tokenomics designed for gas metering and file storage incentives. For information on the differences in the smart contract systems, see [Kontor vs. Alkanes](/docs/resources/alkanes).
