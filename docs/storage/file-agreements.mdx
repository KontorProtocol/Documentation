...
sequenceDiagram
    ...
    U->>U: Prepare File<br/>(Erasure + Merkle)
    U->>SN: Distribute File Data
    U->>BC: Create Agreement Tx
    BC->>IX: Mine Block
    IX->>IX: Create Inactive Agreement
    
    rect rgb(240, 240, 240)
        SN->>BC: Join Agreement Tx
        BC->>IX: Mine Block
        IX->>IX: Add Node to Agreement
        IX->>IX: Activate ($$ N_f > n_{min}$$)
    end
```
