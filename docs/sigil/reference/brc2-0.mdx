...
2. Has more limited interoperability with Bitcoin itself
3. Does not have a native gas currency

Kontor implements a WebAssembly-based execution environment with typed WIT interfaces, purpose-built for Bitcoin rather than ported from Ethereum. Kontor also uses a hybrid UTXO model that supports deep interoperability with Bitcoin via the native chaining of PSBTs and atomic swaps. 
