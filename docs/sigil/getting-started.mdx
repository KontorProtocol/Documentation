...
1. **Unzip the Package**: Extract the ZIP file provided to you via messaging (e.g., Telegram). The extracted directory has the following structure:
   ...
   - `example-contracts/`: Contains a set of example contracts that highlight some of Sigil's most important features.

2. **Navigate to the Example Contract**:
   ```bash
   cd example-contracts/hello-world
   ```
...
- Work directly in the `hello-world/` directory to experiment with the example contract. Avoid opening the entire `example-contracts/` folder in your editor as the multiple contract workspaces contained within will confuse the Rust language server.
