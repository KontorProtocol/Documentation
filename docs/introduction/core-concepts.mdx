...
Four fundamental concepts underpin how Kontor works: embedded consensus, the dual-token model, the hybrid UTXO model, and atomic swaps.
</div>
...
For architectural details, see [Architecture and Consensus](/docs/kontor/architecture).
...
Kontor enables single-confirmation atomic swaps between BTC, KOR, and assets from other Bitcoin metaprotocols (Ordinals, Runes, Counterparty). Users can chain a BTC-to-KOR swap directly with a contract call in a single Bitcoin transaction, making interaction seamless.
